...
function ch1RecordMistake(lesson,question,answer){state.mistakes.push({lesson,question,answer,ts:Date.now()});state.mistakes=state.mistakes.slice(-80);try{window.KitsuneLearning?.recordTopicSkill?.(lesson,false);window.KitsuneLearning?.recordError?.({topicId:lesson,question,student:answer})}catch(e){}}
...
"1-5":{
 ...
 formula:`(a/b) · (c/d) = ac/bd;   (a/b)ⁿ = aⁿ/bⁿ.`,
...
window.ch1CheckAnswer=(id,i)=>{const e=lessonData[id].exercises[i],input=document.querySelector(`#ans-${id}-${i}`),fb=document.querySelector(`#fb-${id}-${i}`),ok=ch1AnswerMatches(input.value,e.a);state.attempts++;if(ok){state.correct++;try{window.KitsuneLearning?.recordTopicSkill?.(id,true);window.KitsuneLearning?.recordSuccess?.(id)}catch(e){}state.solved[`${id}-${i}`]=true;fb.className="feedback ok";fb.textContent="✅ Верно! Отлично.";const box=document.querySelector(`[data-ex="${id}-${i}"]`);if(box){box.classList.remove("success-flash");void box.offsetWidth;box.classList.add("success-flash");setTimeout(()=>box.classList.remove("success-flash"),900)}ch1TouchActivity();ch1UpdateMastery(id)}else{fb.className="feedback bad";fb.textContent="Пока не так. Попробуй ещё раз или открой подсказку.";ch1RecordMistake(id,e.q,input.value)}saveChapterState()};
window.ch1CheckChallenge=id=>{const e=lessonData[id].challenge,input=document.querySelector(`#ans-${id}-challenge`),fb=document.querySelector(`#fb-${id}-challenge`),ok=ch1AnswerMatches(input.value,e.a);state.attempts++;if(ok){state.correct++;try{window.KitsuneLearning?.recordTopicSkill?.(id,true);window.KitsuneLearning?.recordSuccess?.(id)}catch(e){}state.solved[`${id}-challenge`]=true;fb.className="feedback ok";fb.textContent="🌟 Верно! Сложное задание покорено.";ch1TouchActivity();ch1UpdateMastery(id)}else{fb.className="feedback bad";fb.textContent="Пока не получилось. Подсказка даст направление.";ch1RecordMistake(id,e.q,input.value)}saveChapterState()};
