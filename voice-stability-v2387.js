/* =====================================================================
   Kitsune v2.3.0-beta.3.8.7 · iPHONE SECOND DICTATION FIX
...
(() => {
  ...
  const VERSION="2.3.0-beta.3.8.7";
  ...
  let systemSpeechPoll=null;
  let systemSpeechWatchdog=null;
  let activeSystemFinish=null;
  ...
  function setStatus(text,kind=""){
    ...
    const status=document.querySelector("#v2387VoiceStatus");
    if(status){
  ...
  function clearSystemSpeechTimers(){
    clearInterval(systemSpeechPoll);
    clearTimeout(systemSpeechWatchdog);
    systemSpeechPoll=null;
    systemSpeechWatchdog=null;
  }

  function stopSystemSpeech({runDone=false}={}){
    const finish=activeSystemFinish;
    activeSystemFinish=null;
    clearSystemSpeechTimers();
  ...
  function speakSystemIOS(text,opts={}){
    ...
    /* iOS Safari/PWA sometimes fails to fire SpeechSynthesisUtterance.onend.
       beta.3.8.6 then left systemSpeaking=true forever, so the second press
       of «Говорить» returned immediately and looked like a dead microphone. */
    stopSystemSpeech();
    ...
    const finish=(reason="end")=>{
      ...
      if(!iosInterruptedRecovery&&handsFreeEnabled()&&dialogOpen()&&!document.hidden){
        ...
        retryTimer=setTimeout(()=>{
          if(!sessionBusy&&!recording&&!systemSpeaking&&dialogOpen()){
            startUnified(false,false).catch(()=>{});
          }
        },700);
      }
    };

    activeSystemFinish=finish;
    ...
    /* Safari fallback #1:
       after a short grace period, watch the actual synth state.
       If speech has stopped but onend never arrived, unlock the microphone. */
    systemSpeechPoll=setInterval(()=>{
      if(finished)return;
      ...
      if(!speaking&&!pending){
        finish("state-watchdog");
      }
    },250);

    /* Safari fallback #2: absolute guard against a permanently stuck flag. */
    const maxMs=Math.max(
      7000,
      Math.min(32000,2500+String(text||"").length*95)
    );
    systemSpeechWatchdog=setTimeout(
      ()=>finish("timeout-watchdog"),
      maxMs
    );
  ...
  async function startUnified(fromRetry=false,manualTrigger=false){
    if(!enabled){
    ...
    if(manualTrigger&&systemSpeaking){
      /* The user explicitly pressed the mic: interrupt lightweight iOS TTS
         and immediately give the microphone back instead of silently ignoring
         the button. */
      stopSystemSpeech();
      await sleep(140);
    }

    if(sessionBusy||systemSpeaking||document.hidden||!dialogOpen()){
      if(manualTrigger&&systemSpeaking){
        setStatus("Завершаю голос Kitsune… нажми «Говорить» ещё раз через мгновение.","warn");
      }
      return false;
    }
  ...
  function updateSettingsUi(){
    const enable=document.querySelector("#v2387VoiceEnabled");
    if(enable)enable.checked=enabled;

    const accurate=document.querySelector("#v2387DesktopAccurate");
    if(accurate){
      accurate.checked=desktopAccurate;
      accurate.disabled=isMobileLike();
    }

    const prepare=document.querySelector("#v2387PrepareVoice");
    if(prepare){
  ...
  function injectSettings(){
    ...
    if(!host||document.querySelector("#v2387VoiceStability"))return;

    const block=document.createElement("div");
    block.id="v2387VoiceStability";
    ...
    block.innerHTML=`
      ...
      <label style="display:flex;gap:7px;align-items:flex-start;margin-top:9px;cursor:pointer">
        <input id="v2387VoiceEnabled" type="checkbox">
      ...
      <label style="display:flex;gap:7px;align-items:flex-start;margin-top:8px;cursor:pointer">
        <input id="v2387DesktopAccurate" type="checkbox">
      ...
      <div id="v2387VoiceStatus" style="margin:9px 0;opacity:.82">
        ${isIOSLike()
          ?"iPhone: Whisper Tiny будет загружен один раз и останется до закрытия диалога."
          :"Новый голосовой ввод готов к подготовке."}
      </div>

      <button id="v2387PrepareVoice" type="button" class="v15-action primary-action">
        ⬇ Подготовить единый голосовой ввод
      </button>
    ...
    block.querySelector("#v2387VoiceEnabled")?.addEventListener("change",event=>{
      enabled=!!event.target.checked;
      writeBool(ENABLED_KEY,enabled);
      updateSettingsUi();
    });

    block.querySelector("#v2387DesktopAccurate")?.addEventListener("change",async event=>{
    ...
    block.querySelector("#v2387PrepareVoice")?.addEventListener("click",async()=>{
      try{
  ...
  function patchApi(){
    ...
    if(!api||api.__unifiedVoiceV2387)return false;
    ...
    api.start=(fromRetry=false)=>startUnified(!!fromRetry,false);
  ...
  function install(){
    ...
    const timer=setInterval(()=>{
      injectSettings();
      observeDialogLifecycle();
      if(document.querySelector("#v2387VoiceStability")&&dialogObserver){
        clearInterval(timer);
      }
    },700);
  ...
  window.KitsuneUnifiedVoice={
    ...
    start:()=>startUnified(false,false),
    stop:stopUnified,
    status:()=>({
      enabled,
      ios:isIOSLike(),
