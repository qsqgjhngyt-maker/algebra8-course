...
## PWA update events (v1.11.9+)

При включённой взрослым аналитике могут отправляться только технические события:
- `pwa_update_available`
- `pwa_update_apply_click`

Payload содержит только разрешённое поле `version`. Учебные данные, ответы, чат и локальный прогресс в эти события не включаются.
